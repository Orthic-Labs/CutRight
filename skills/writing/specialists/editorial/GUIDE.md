...
|---|---|
| Blog post, article page, SEO post, post audit, blog publish QA | blogs lane — excluded in CutRight v2 (`imports/v2/exclusions/writing.json`) |
| Landing-page copy, sales copy, offer, bio, DM/email script, CTA, conversion copy | `copywriting` |
| Brand identity, voice system, guidelines, naming direction | `cutright://skill/brand-identity` |
| Website/page design or app UI | `cutright://skill/designer` |

Use `writing-pro` only when the primary deliverable is prose rather than SEO/publish gates or conversion copy.

## Always start with

1. `cutright://skill/brand {"brand_code":"<DD|RH|SS>"}` — voice lock (typed result: BrandCard)
...
For social-platform calendars/optimization, route to `cutright://skill/social`. For conversion ad copy + hooks, route to `copywriting`. The upstream blog lane is excluded in CutRight v2.

## Internal Writing Council

Use this before drafting substantial writing. For routed work, inherit the more specific role pass from the matching lane (the upstream email lane is excluded in CutRight v2); this section covers direct long/short-form output.
...
### Email
- The email lane is excluded in CutRight v2 (`imports/v2/exclusions/writing.json`); the rules below are upstream provenance
...
### About page / Bio
- The upstream profile lane is excluded in CutRight v2; bio copy routes to `specialists/copywriting/GUIDE.md`
