...
Anjela Petkova's 5-prompt sequence for scroll-stopping content. Adapted for one-pass generation. Includes the 0.3s filter as a final gate (also reusable standalone inside copywriting ad copy, writing scripts, social twitter framing, and content-strategy review).
...
- For brand work, apply `cutright://skill/brand {"brand_code":"<X>"}` voice rules (BrandCard)
...
- `specialists/copywriting/GUIDE.md` — final gate before delivering hook/headline/ad copy
- `references/script.md` — verify reel/Shorts opening
- `cutright://skill/social` (twitter reference) — match the algorithm framing
