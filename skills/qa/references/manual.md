...
These runners use installed Chrome/Edge directly through headless flags and raw CDP. They do not use Playwright or Puppeteer and download nothing. In CutRight v2 they execute as typed capabilities (`qa.qa`, `qa.qa_functional`, `qa.qa_shot`) inside the signed qa runtime pack; the base skill requires no Node installation on PATH.
