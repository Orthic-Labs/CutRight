...
- **Brand gate:** brand voice or `cutright://skill/brand-identity` exists; if not, extract a lightweight voice/promise first.
...
1. Run `cutright://skill/brand {"brand_code":"<DD|RH|SS|TS>"}` for branded work (typed result: BrandCard).
2. Identify the platform and task type: create / plan / optimize / review. **For optimize/review of live content, ask the operator for the platform analytics first** (IG Insights, YouTube Studio, X analytics — provided as screenshots/exports; CutRight v2 excludes account connectors) — "optimise my IG" without reach/saves numbers is guesswork; ground it or label it hypothetical.
...
5. **Visual-asset handoff.** For carousels, pins, thumbnails, or story graphics, produce the exact copy + slide-by-slide structure, then offer to route to `cutright://skill/designer` (static) for the actual assets — don't leave "Slide 1: [hook]" as the deliverable. Video scripts → `cutright://skill/content` / the video pipeline.
6. **Distribution boundary.** Source material already exists? Repurpose it, don't rewrite from scratch. Paid ads/budgets/funnel and deep search optimisation belong to upstream `marketing`/`seo` lanes (not vendored into CutRight v2). A CTA link must resolve — verify it host-side before delivery — and, for conversion, carry UTM params; never ship a dead or untracked link. Posting, scheduling, and account connectors stay excluded: publishing plans are data for the operator.

Existing Instagram carousel slide extraction routes to `cutright://skill/qa {"mode":"capture"}` (the upstream content carousel branch is excluded); social owns concept,
copy, slide sequence, platform fit, publishing strategy, & optimization.

Stunning Strangers is a passion project. Do not apply growth, ads, SEO, or commercialization framing to SS unless Adrian explicitly asks.
