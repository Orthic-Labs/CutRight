...
This skill uses the same mechanism as the `cutright://skill/designer` website surface: product/brand truth first, a generative signature mechanism, three divergent renderings, automated hard gates, visual/design QA, then human approval and guidelines.
...
downstream skills (`cutright://skill/designer`, `cutright://skill/qa {"mode":"visual_review"}`, and the upstream `architect` skill — not vendored into CutRight) consume `.brand/tokens.json`
...
**Also emit a machine-readable `<repo>/.brand/tokens.json`** — the deterministic source of truth that downstream skills consume (`cutright://skill/designer` builds UI from it, `cutright://skill/qa {"mode":"visual_review"}` checks a rendered surface against it, and the upstream `architect` skill — not vendored into CutRight — would plan the theme-variable refactor from it). Human `BRAND.md` is for people; this JSON is for agents. Minimum shape:
...
- Apply the brand to a new site/app UI → `cutright://skill/designer` (consumes `.brand/tokens.json`).
- Audit how an existing UI adheres to the new brand → `cutright://skill/qa {"mode":"visual_review"}` (checks the rendered surface against the tokens).
- Refactor the codebase theme variables to match the tokens → the upstream `architect` skill plans it (not vendored into the CutRight corpus; track as a manual refactor).
