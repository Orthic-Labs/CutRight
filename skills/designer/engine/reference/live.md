...
> **CutRight v2 contract:** live-mode variants do not mutate the workspace directly from this skill.
> Each generated variant is emitted as an AssetDelivery record on the CutRight job plane; the dev
> server / browser helper described below runs only inside the signed runtime pack via typed
> capabilities (`cutright://capability/designer.live`, `designer.live_poll`, ...).

Execute in order. No step skipped, no step reordered.

1. `live.mjs`: boot. If the request names or implies a file, route, or app inside a monorepo, infer the concrete path and run `cutright://capability/designer.live --target <path>` instead; then run the rest of this live session from the returned `projectRoot`.
...
```bash
cutright://capability/designer.live
```
...
```
LOOP:
  cutright://capability/designer.live-poll   # default long timeout; no --timeout=
...
```
cutright://capability/designer.live-poll --stream   # stays running; one JSON line per event
...
```bash
cutright://capability/designer.live-status
cutright://capability/designer.live-resume --id SESSION_ID
cutright://capability/designer.live-complete --id SESSION_ID
```
...
```bash
cutright://capability/designer.live-insert --id EVENT_ID --count EVENT_COUNT --position after \
...
```bash
cutright://capability/designer.live-wrap --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
```bash
cutright://capability/designer.live-poll --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
cutright://capability/designer.live-poll --reply EVENT_ID error "Short reason"
```
...
```bash
cutright://capability/designer.live-poll --reply EVENT_ID steer_done ["Optional short note for a browser toast"]
```
...
```bash
cutright://capability/designer.live-server stop
```
...
```bash
cutright://capability/designer.detect-csp
```
