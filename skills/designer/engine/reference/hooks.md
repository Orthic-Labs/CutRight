...
> **CutRight v2 note:** the third-party harness manifests listed above (`.claude/`, `.codex/`, `.cursor/`, `.github/`) are recorded as upstream provenance only; CutRight installs no external harness files. Hook administration is exposed solely as the typed capability `cutright://capability/designer.hook-admin`.
...
2. Invoke the admin script and pass the user's output through verbatim:
   ...
   ```bash
   cutright://capability/designer.hook-admin <action> [args...]
   ```

3. If `<action>` is `off`, follow up with a one-line note: "Done. New edits will not trigger the design hook in this project until you run `cutright://skill/designer hooks on`."
...
```bash
cutright://capability/designer.hook-admin ignore-value overused-font Inter --shared --reason "User confirmed Inter is intentional"
```

Example intentional motion exception:

```bash
cutright://capability/designer.hook-admin ignore-value bounce-easing bounce-ball --shared --reason "User confirmed ball bounce animation is intentional"
```

Example whole-rule font exception:

```bash
cutright://capability/designer.hook-admin ignore-rule overused-font --all-values --reason "User asked to ignore overused fonts generally"
```

Example file-scoped exception:

```bash
cutright://capability/designer.hook-admin ignore-file "src/legacy/Card.tsx"
```
...
- The hook is bundled with the Impeccable skill and installed through project-local manifests: `.claude/settings.local.json`, `.codex/hooks.json`, `.cursor/hooks.json`, and `.github/hooks/designer.json` (upstream provenance; under CutRight v2 no such manifests are written — see the note above). On Codex, the user must approve the hook via `/hooks` the first time. On Cursor, confirm hooks are enabled under Settings -> Hooks. On GitHub Copilot, the CLI loads `.github/hooks/designer.json` once it is committed to the repository's default branch, and the cloud agent reads it from the repo directly.
...
- If the user asks to "disable the hook" globally, lead with `cutright://skill/designer hooks off` (persistent for this project; writes `hook.enabled: false` to config). The legacy `IMPECCABLE_HOOK_DISABLED=1` env var also works as a one-shot override that follows the shell.
