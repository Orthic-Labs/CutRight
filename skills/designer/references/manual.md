...
MAY_ADD_TASKS: NO
MAY_CALL_SKILLS: cutright://skill/qa (visual_review), cutright://skill/brand, cutright://skill/brand-identity, cutright://skill/content
...
| Review an existing rendered surface without implementation | `cutright://skill/qa {"mode":"visual_review"}` |
| Create or evolve the underlying brand identity | `cutright://skill/brand-identity` |
...
1. Load the relevant `cutright://skill/brand` before brand-specific work. **If `<repo>/.brand/tokens.json` exists (emitted
   by `cutright://skill/brand-identity`), read it and apply its exact hex/OKLCH values, font stacks, and voice arrays
...
6. Run `cutright://skill/qa {"mode":"visual_review"}` and the hidden `cutright://skill/qa` functional workflow before making a rendered ship claim. Adrian's eyes
   approve visual taste.
7. **Output contract:** this skill performs no direct workspace mutation; every produced artifact is
   emitted as an AssetDelivery record (answering an AssetRequest / RenderSampleRequest) on the
   CutRight job plane.
...
2. Hard vs soft: brand tokens/palette/fonts, a11y floors, banned vocabulary are hard constraints —
   reject on violation. Tone/density/risk are soft — score down. Brand rules (`brand-pack/<brand_code>/restrictions.md (upstream workspace file .claude/rules/brands.md)`)
   are always hard.
...
- Words as the primary deliverable -> `cutright://skill/writing`.
- Images/video/media production rather than design of a surface -> `cutright://skill/content`.
- Paid-media campaign strategy -> the upstream `ads` skill (not vendored into the CutRight corpus); this skill may still produce its creative assets.
